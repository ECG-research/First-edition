...
class Cross_attention(nn.Module):
    def __init__(self,query_dim,num_head=2,dropout = 0.2,batch_first= True):
        super().__init__()
    ...
    def forward(self,query0,query1):
        ...
        return torch.cat((feature1,feature0,query0),dim = 1)
...
#         return self.relu(torch.cat((feature1,feature0),dim = 1))

class SEresnet(nn.Module):

    def __init__(self, inplanes, planes, stride=1, ex=2, reduction = 16):
        super().__init__()
        self.conv1 = conv(inplanes, planes, kernel_size=5, stride = stride)
        self.bn1 = nn.BatchNorm1d(planes)
        self.conv2 = conv(planes, planes * ex, kernel_size=5, stride = stride)
        self.bn2 = nn.BatchNorm1d(planes * ex)
        self.relu = nn.ReLU(inplace=True)
        # residual
        self.conv_res = conv(inplanes, planes * ex, kernel_size = 5, stride = stride)
        self.maxpool = nn.MaxPool1d(kernel_size = 5,stride = stride,padding = 2)
        # SE
        self.global_pool = nn.AdaptiveAvgPool1d(1)
        self.conv_down = conv(planes * ex, planes * ex//reduction, kernel_size=1)
        self.conv_up = conv(planes *ex//reduction, planes * ex, kernel_size=1)
        self.sig = nn.Sigmoid()
...
class Modified_version(nn.Module):
    ...
    def __init__(self,num_leads = 3, num_classes=5, input_channels=1, kernel_size=40, depth=3, bottleneck_size=32, nb_filters=32, 
        ...
        self.Incep_layers = nn.ModuleList([InceptionBackbone(input_channels=input_channels, kss=kernel_size, depth=depth,
                    bottleneck_size=bottleneck_size, nb_filters=nb_filters, use_residual=use_residual) for i in range(num_leads)])
    ...
    def forward(self,x0,x1,x2,metadata):
        ...
        #spatial
        enhanced_feature = self.spatial(enhanced_feature)
        #classify
