...
# f1 = BinaryF1Score()
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max = 6,eta_min=1e-4)
...
train_data = DataLoader(TensorDataset(x_train,y_train), batch_size=256, shuffle=True)

val_data = DataLoader(TensorDataset(x_val,y_val), batch_size =256, shuffle=False)

test_data = DataLoader(TensorDataset(x_test,y_test), batch_size =256, shuffle=False)
...
print("Val_inception_score:{}".format(val_score_all))
print("Val_inception_acc:{}".format(val_acc_all))
print("Val_inception_auc:{}".format(val_auc_all))