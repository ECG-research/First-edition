...
# branch network
branch_stft = model.Branch().to(device)

# evaluation methods
criterion = nn.functional.binary_cross_entropy_with_logits
optimizer = optim.Adam(branch_stft.parameters(), lr=1e-4)
# f1 = BinaryF1Score()
...
# dataloader
train_data = DataLoader(TensorDataset(stft_train,sletrain,ytrain), batch_size=128, shuffle=True)
val_data = DataLoader(TensorDataset(stft_val,sleval,yval), batch_size = 128, shuffle=False)
test_data = DataLoader(TensorDataset(stft_test,sletest,ytest), batch_size = 128, shuffle=False)
...
for t in range(1):
    for epoch in tqdm(range(num_eporch)):
        ...
        for inputs,metadata,labels in tqdm(train_data):
            ...
            outputs = branch_stft(inputs.float(),metadata.float())
...
torch.save(branch_stft.pm.state_dict(),"stft_pm_test")
torch.save(branch_stft.classifier.state_dict(),"pseudo_test")
